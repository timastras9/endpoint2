...
class LLM:
    ...
    async def _make_request(
        ...
        # Set large context window for Ollama models
        if self.config.model_name and "ollama" in self.config.model_name.lower():
            completion_args["api_base"] = os.getenv("LLM_API_BASE", "http://localhost:11434")
            completion_args["num_ctx"] = 32768
            # Also try passing via extra params for litellm
            completion_args["extra_body"] = {"options": {"num_ctx": 32768}}
